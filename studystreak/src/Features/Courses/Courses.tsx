  ...
  const platforms = [
    {
      ...
      color: 'bg-gray-50 dark:bg-slate-800/60'
    },
    {
      name: 'Khan Academy',
    ...
    },
    {
      ...
      color: 'bg-gray-100 dark:bg-slate-800/80'
    },
    {
      name: 'Udemy',
    ...
    <div className="space-y-12 p-6 max-w-7xl mx-auto">
      ...
      <section className="relative">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-bold text-slate-800 dark:text-white">Learning Platforms</h2>
            <p className="text-slate-600 dark:text-slate-400 mt-1">Continue your learning journey with these educational platforms</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {platforms.map((platform) => (
            <div key={platform.name}>
              ...
              <Card className={`relative overflow-hidden h-56 border border-gray-200 dark:border-slate-700 ${platform.color} shadow-sm 
                ...
                {!loadedImages[platform.name] && (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-100/80 dark:bg-[#0A1220]/40 z-10">
                    ...
                    <div className="h-full flex flex-col relative z-10">
                      ...
                      <div className="mt-auto">
                        <div className="flex items-center justify-between">
                          <div>
                            {/* Platform name */}
                            <div className="text-lg font-medium text-slate-900 dark:text-white">{platform.name}</div>
                            {/* Platform description */}
                            <div className="text-sm text-slate-700 dark:text-slate-300">{platform.description}</div>
                          </div>
                          {/* External link icon with hover effect */}
                          <ExternalLink className="w-5 h-5 text-slate-600 dark:text-slate-300 group-hover:text-blue-500 dark:group-hover:text-white transition-colors" />
                        </div>
                      </div>
                    </div>
                  </a>
